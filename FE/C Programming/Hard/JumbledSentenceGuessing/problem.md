...
5. **Winning Condition**: The program ends when the user's guess exactly matches the original sentence.

## Examples
```text
Input: outside playing children the are (Jumbled)
Output: Correct Guess: the children are playing outside
```
```text
Input: homework my the cat ate (Jumbled)
Output: Correct Guess: the cat ate my homework
```
```text
